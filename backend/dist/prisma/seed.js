...
function inferCategory(tags) {
    ...
    if (lower.some((t) => ['italian', 'pasta', 'pizza'].includes(t)))
        return 'Italian';
    if (lower.some((t) => ['mexican', 'street'].includes(t)))
        return 'Mexican';
    if (lower.some((t) => ['indian', 'curry'].includes(t)))
        return 'Indian';
    if (lower.some((t) => ['thai', 'vietnamese', 'pho'].includes(t)))
        return 'Southeast Asian';
    if (lower.some((t) => ['japanese', 'ramen', 'sushi'].includes(t)))
        return 'Japanese';
    if (lower.some((t) => ['korean'].includes(t)))
        return 'Korean';
    if (lower.some((t) => ['french'].includes(t)))
        return 'French';
    if (lower.some((t) => ['american', 'bbq'].includes(t)))
        return 'American';
    if (lower.some((t) => ['british'].includes(t)))
        return 'British';
    if (lower.some((t) => ['spanish'].includes(t)))
        return 'Spanish';
    if (lower.some((t) => ['greek', 'mediterranean'].includes(t)))
        return 'Mediterranean';
    if (lower.some((t) => ['middle eastern'].includes(t)))
        return 'Middle Eastern';
    if (lower.some((t) => ['breakfast', 'brunch'].includes(t)))
        return 'Breakfast';
    if (lower.some((t) => ['dessert', 'chocolate', 'baking', 'sweet'].includes(t)))
        return 'Dessert';
    if (lower.some((t) => ['soup', 'comfort'].includes(t)))
        return 'Soup';
    if (lower.some((t) => ['salad'].includes(t)))
        return 'Salad';
    if (lower.some((t) => ['vegan', 'vegetarian', 'bowl'].includes(t)))
        return 'Plant-based';
    if (lower.some((t) => ['asian', 'stir-fry', 'dumpling'].includes(t)))
        return 'Asian';
    if (lower.some((t) => ['fish', 'seafood'].includes(t)))
        return 'Seafood';
...
function pickLikers(recipeId, authorUserId, allUsers) {
    const others = allUsers.filter((u) => u.id !== authorUserId);
    const n = Math.min(others.length, 3 + hashPick(recipeId, 5));
    return [...others]
        .sort((a, b) => hashPick(recipeId + a.id, 1e9) - hashPick(recipeId + b.id, 1e9))
        .slice(0, n);
}
async function ensureClerkUser(clerk, email, firstName, lastName) {
...
async function wipeAppDataExceptProtectedUser() {
    ...
    if (keep) {
        ...
        console.log(`  No user with ${PROTECTED_EMAIL} — all users removed.`);
    }
}
async function main() {
    const secret = process.env.CLERK_SECRET_KEY?.trim();
    if (!secret) {
        throw new Error('CLERK_SECRET_KEY is missing in backend/.env — required to create sign-in accounts.');
    }
    await wipeAppDataExceptProtectedUser();
    const clerk = (0, backend_1.createClerkClient)({ secretKey: secret });
    ...
    for (let i = 0; i < createdIds.length; i += 7) {
        ...
        if (saver)
            favRows.push({ userId: saver.id, recipeId: rid });
    }
